...
//
  ...
  std_msgs__msg__String_sub1 pubmsg_str;
  ...
  for(;;)
  {
    // publish message
	  ...
	  //	  pubmsg_3.hall_1 = pubmsg_1.hall_1 + 10000*pubmsg_2.hall_2;
	  	  pubmsg_3.hall_1 = l_enc_op + r_enc_op + 100*abs(pubmsg_1.hall_1) + 10000*100*abs(pubmsg_2.hall_2);

//		  	sprintf(enc_str1, "%d_%d", pubmsg_2.hall_2, pubmsg_1.hall_1);
		  	sprintf(enc_str1, "%d_%d", -1 * motor2.position, motor1.position);
	  	pubmsg_str.enc_str.data = enc_str1;
	  	pubmsg_str.enc_str.size = strlen(enc_str1);
	  	pubmsg_str.enc_str.capacity = sizeof(enc_str1);

	  	strcpy(pubmsg_str.enc_str.data, enc_str1);
...
/*		 rcl_ret_t ret_3 = rcl_publish(&publisher_2, &pubmsg_2, NULL);
		 ...
		 rcl_ret_t ret_str = rcl_publish(&publisher_str, &pubmsg_str, NULL);
